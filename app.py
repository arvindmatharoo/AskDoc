...
section[data-testid="stSidebar"] {
    background: rgba(9,9,18,0.90) !important;
...
.wlc-wrap {
    ...
    padding: 50px 20px 20px; min-height: 72vh;
}
.wlc-card {
...
def _init():
    defaults = {
        ...
        "session_id": "askdoc_s1",  # passed to RunnableWithMessageHistory
...
def render_chat(rag_chain):
    ...
    if user_in:
        ...
        # Show typing indicator, then stream answer
        with st.chat_message("assistant"):
            ...
            placeholder.empty()
            st.markdown(ans)
            copy_btn(ans, uid="live")   # copy button on the live response too
